...
*/
?>
<title><?php echo "$site_adi"." - "."Cevaplanmış Destek Biletlerim"; ?></title>
</head>
<body>
<?php
...
echo '<b><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#FF0000">Bekleyen Destek Biletlerim:</font></b><br />';
...
$toplam = mysql_query("SELECT * FROM tickets where durum='Kapalı' and reporter='$reporter'", $baglanti_pointer);
...
$tickets = mysql_query("select * from tickets where durum='Kapalı' and reporter='$reporter' order by no limit $baslangic,$bitis", $baglanti_pointer);
//Eğer sayfamız 1'den büyükse o zaman geri linki oluşturup ekrana yazdırıyoruz. 
if($no>1){
echo '<a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=cevaplanmis&no='.($no-1).'>Geri</a> | ';
}
...
for($i=0; $i<$toplam_sayfa; $i++){
	...
	echo' <a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=cevaplanmis&no='.($i+1).'>'.($i+1).'</a> ';
	}
}
//Eğer toplam sayfamız şu anda bulunduğumuz sayfanın bir fazlasından daha fazla ise o zaman ileri linki oluşturuyoruz.
if($toplam_sayfa>$no){
echo'| <a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=cevaplanmis&no='.($no+1).'>İleri</a>';
}
//VERİLERİ YAZDIRMA:
//Tablo oluşturuluyor.
echo '<center><table border="1">';
echo "<tr><td><font face='verdana' size='2' color='red'>&nbsp;Bilet No&nbsp;</font></td><td>&nbsp;<font face='verdana' size='2' color='red'>Başlık&nbsp;</font></td><td>&nbsp;<font face='verdana' size='2' color='red'>Kategori&nbsp;</font></td><td>&nbsp;<font face='verdana' size='2' color='red'>İşlemler&nbsp;</font></td></tr>";
while($ticket = mysql_fetch_array($tickets)){ 
	...
	<input type='submit' name='goster' value='Göster'><input type='submit' name='ac' value='Bileti Yeniden Aç'>
	</form>
	</center>
	</td></tr>";
}
