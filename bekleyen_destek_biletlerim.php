...
*/?>
<title><?php echo "$site_adi"." - "."Bekleyen Destek Biletlerim"; ?></title>
</head>
<body>
<?php
...
$toplam = mysql_query("SELECT * FROM tickets where durum='Açık' and reporter='$reporter'", $baglanti_pointer);
...
$tickets = mysql_query("select * from tickets where durum='Açık' and reporter='$reporter' order by no limit $baslangic,$bitis", $baglanti_pointer);
//Eğer sayfamız 1'den büyükse o zaman geri linki oluşturup ekrana yazdırıyoruz. 
if($no>1){
echo '<a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=bekleyen&no='.($no-1).'>Geri</a> | ';
}
...
for($i=0; $i<$toplam_sayfa; $i++){
	...
	echo' <a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=bekleyen&no='.($i+1).'>'.($i+1).'</a> ';
	}
}
//Eğer toplam sayfamız şu anda bulunduğumuz sayfanın bir fazlasından daha fazla ise o zaman ileri linki oluşturuyoruz.
if($toplam_sayfa>$no){
echo'| <a href='.$PHP_SELF.'?sayfa=destek_biletlerim&goster=bekleyen&no='.($no+1).'>İleri</a>';
}
//VERİLERİ YAZDIRMA:
//Tablo oluşturuluyor.
...
while($ticket = mysql_fetch_array($tickets)){ 
	...
	<input type='submit' name='goster' value='Göster'><input type='submit' name='kapat' value='Bileti Kapat'>
	</form>
	</center>
	</td></tr>";
}
