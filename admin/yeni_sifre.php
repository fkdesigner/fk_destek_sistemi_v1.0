...
$bilgi_kontrol = mysql_query("select * from yoneticiler where e_mail='$e_mail' and guvenlik_kodu='$guvenlik_kodu'", $baglanti_pointer);
$bilgi_kontrol_sonucu = mysql_num_rows($bilgi_kontrol);
if ($bilgi_kontrol_sonucu == 0){
echo '<br><center><b><font face="verdana" size="2" color="red">Girdiğiniz e-posta adresi ya da güvenlik kodu yanlış.</font><br><br>';
echo '<a href="sifremi_unuttum.php">Geri Dön</a></b></center>';
}
else {
while($admin_bilgi = mysql_fetch_array($bilgi_kontrol)) { 
$admin_username = $admin_bilgi['username'];
$admin_id = $admin_bilgi['no'];
}
...
$admin_password = sha1($sifre);
//Şifreyi veritabanına kaydediyoruz.
$sifre_guncelle = mysql_query ("update yoneticiler set userpass='$sifre' where no='$admin_id'", $baglanti_pointer);
...
<table width="439" height="225" border="1" align="center">
  <tr>
	...
	<i><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#CCCC00"><?php echo "$site_aciklama";?></font></i><br />
	</center><br /><br />
<form method="post" action="<?php echo $_SERVER["SCRIPT_NAME"]."?e_mail="."$e_mail"."&guvenlik_kodu="."$guvenlik"; ?>">
<div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000066">Kullanıcı : <font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="black"><?php echo $admin_username ?></font><br /><br /><font face="Verdana, Arial, Helvetica, sans-serif" size="2" color="#000066">Yeni Şifreniz : </font>
