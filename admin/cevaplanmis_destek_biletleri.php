...
*/
?>
<title><?php echo "$site_adi"." - "."Cevaplanmış Destek Biletleri"; ?></title>
</head>
<body>
<?php
...
$toplam = mysql_query("SELECT * FROM tickets where durum='Kapalı'", $baglanti_pointer);
...
$tickets = mysql_query("select * from tickets where durum='Kapalı' order by no limit $baslangic,$bitis", $baglanti_pointer);
...
if($toplam_sayfa>$no){
echo'| <a href='.$PHP_SELF.'?sayfa=destek_biletleri&goster=cevaplanmis&no='.($no+1).'>İleri</a>';
}
//VERİLERİ YAZDIRMA:
//Tablo oluşturuluyor.
...
while($ticket = mysql_fetch_array($tickets)){ 
	...
	<input type='submit' name='goster' value='Göster'><input type='submit' name='ac' value='Bileti Yeniden Aç'><input type='submit' name='sil' value='Sil'>
	</form>
	</center>
	</td></tr>";
}
